...
export async function GET(
  ...
  const doc = await clientDocuments.getById(docId);
  if (!doc) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const result = await clientDocuments.getDownloadUrl(doc.storage_path);
  ...
  console.log(`[client-documents] download by ${staffEmail}: doc ${docId} (${doc.doc_type})`);
