...
const siteName = process.env.NEXT_PUBLIC_SITE_NAME || "USSP";
...
export default function LoginPage() {
  ...
  const providerLabel = isMicrosoft ? "Sign in with Microsoft" : "Sign in with Google";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-near-black">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-sm text-center">
        ...
        <p className="text-sm text-dark/60 mb-2">by {siteName}</p>
        ...
        <p className="text-xs text-dark/40 mt-6">
          Access restricted to authorized {siteName} users
        </p>
      </div>
    </div>
  );
